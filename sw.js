'use strict';
const CACHE='mosaic64-pwa-v0.6.8-1';
const CORE=[
  './',
  './index.html',
...
self.addEventListener('activate',event=>{
  event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('mosaic64-pwa-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()));
});
